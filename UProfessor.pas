...
type
  ...
  public
    ...
    class function FromLine(const Linha: string; out E: TProfessor): Boolean; static; // cria a partir do texto
  end;
...
class function TProfessor.FromLine(const Linha: string; out E: TProfessor): Boolean;
var
  p: Integer;
  sCod, sNome,sCPF: string;
begin
  ...
  E := TProfessor.Create;            // cria aluno
...
procedure TFProfessores.Carregar;
begin
  if FLista = nil then
...
procedure TFProfessores.Salvar;
begin
  if (FLista <> nil) and (FArquivo <> '') then
    FLista.SaveToFile(FArquivo, TEncoding.UTF8); // salva no disco
end;

procedure TFProfessores.AtualizarListaUI;
begin
  if FLista = nil then Exit;         // se não tem lista, não faz nada
//  lbProfessores.Items.Assign(FLista); // mostra a lista na tela
end;

procedure TFProfessores.LimparCampos;
begin
...
function TFProfessores.IndexPorCodigo(const Cod: Integer): Integer;
var
  i, C: Integer;
  p: Integer;
  Linha: string;
...
//procedure TFProfessores.lbEstudantesClick(Sender: TObject);
//var
//  Linha: string;
//  p: Integer;
//begin
